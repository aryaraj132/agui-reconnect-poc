...
const runtime = new CopilotRuntime({
  agents: {
    default: new LangGraphHttpAgent({
      url: `${BACKEND_URL}/api/v1/template-editor/fe-tools`,
      description: "Template editor with FE tool calls",
    }),
  },
});
...
export const POST = async (req: Request) => {
  ...
  if (body.method === "agent/connect") {
    ...
    const backendResp = await fetch(
      `${BACKEND_URL}/api/v1/template-editor/fe-tools`,
      {
        method: "POST",
  ...
  const { handleRequest } = copilotRuntimeNextJSAppRouterEndpoint({
    ...
    endpoint: "/api/copilotkit/template-editor/fe-tools",
  });
